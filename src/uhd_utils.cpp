...
UHD_UTILS::UHD_UTILS(const std::string & args): device_args(args) {
	find_devices();
};

UHD_UTILS::~UHD_UTILS() {};

void UHD_UTILS::find_devices() {
	const uhd::device_addr_t args(device_args);
...
void UHD_UTILS::uhd_print_devices() {
	int i = 0;
...
device_addrs_filtered_t UHD_UTILS::uhd_get_devices() {
