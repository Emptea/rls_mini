...
/**
 * Read a hex file once and extract data for channel.
 ...
 * @param buffer                Output buffer (uint8_t*), holds int16_t data
 * @param ch_num                Number of channel to extract
 ...
 */
int misc_read_channel_from_file(const char * filename, uint8_t * buffer, size_t ch_num, size_t buffer_size_bytes, size_t sample_offset) {
	...
	while (sample_count < max_samples && fgets(line, sizeof(line), fp)) {
		...
		for (size_t pos = 0; pos + 64 <= line_len && sample_count < max_samples; pos += 64) {
			...
			int re_offset = (ch_num * 2) * 4; // channel * 2 values * 4 chars
			int im_offset = (ch_num * 2 + 1) * 4;
			...
			buf16[sample_count * 2]     = re_val;
			buf16[sample_count * 2 + 1] = im_val;
			sample_count++;
		}
	}
